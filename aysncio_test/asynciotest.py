...
@asyncio.coroutine
def hello(index):
    print('hello world! index=%s,thread=%s' %(index,threading.current_thread()))
    yield from asyncio.sleep(1)
    print('hello again! index=%s,thread=%s' %(index,threading.current_thread()))

loop = asyncio.get_event_loop()
tasks = [hello(2),hello(1)]
